...
const API_ENDPOINTS = ['/api/', '/api/results', '/api/progress', '/api/sync'];
const OFFLINE_PAGES = ['/الرئيسية.html', '/تمرين-المجسمات-ثلاثية-الأبعاد.html', '/حساب-النسبة-المئوية.html', '/تحدي-الأشكال.html'];

self.addEventListener('install', (event) => {
    console.log('🔧 Service Worker installing...');
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then(cache => cache.addAll(CACHE_VERSIONS[CACHE_NAME]))
            .then(() => {
            ...
            caches.match(event.request)
                .then(response => response || fetch(event.request).then(response => {
                    ...
                    if (event.request.mode === 'navigate') {
                        return caches.match('/الرئيسية.html').then(offlineResponse => offlineResponse || new Response('لا يوجد اتصال بالإنترنت'));
                    }
...
self.addEventListener('message', (event) => {
    ...
    if (event.data.action === 'updateCache') updateCache();
    if (event.data.action === 'clearCache') clearCache();
});
...
self.addEventListener('sync', (event) => {
    if (event.tag === 'sync-results') event.waitUntil(syncResults());
    if (event.tag === 'sync-progress') event.waitUntil(syncProgress());
});

self.addEventListener('periodicsync', (event) => {
    if (event.tag === 'update-check') event.waitUntil(checkForUpdates());
});
...
async function handleApiRequest(request) {
    ...
    try {
        ...
        return new Response(
            JSON.stringify({ error: 'Network error', message: 'You are offline' }),
...
function shouldCache(request) {
    ...
    const cacheablePaths = ['/الرئيسية.html', '/تمرين-المجسمات-ثلاثية-الأبعاد.html', '/حساب-النسبة-المئوية.html', '/تحدي-الأشكال.html', '/manifest.json'];
...
async function updateCache() {
    ...
    for (const url of CACHE_VERSIONS[CACHE_NAME]) {
        try {
    ...
    clients.forEach(client => client.postMessage({ action: 'cacheUpdated', timestamp: new Date().toISOString() }));
}
...
async function clearCache() {
    ...
    const clients = await self.clients.matchAll();
    clients.forEach(client => client.postMessage({ action: 'cacheCleared', timestamp: new Date().toISOString() }));
}
